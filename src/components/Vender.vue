...
<template>
  <div>
      <h2>Hace tu venta de criptomonedas</h2>
      ...
      <div id="groupbox"> 
        <label>Indica la cantidad de {{crypto_code}} a vender:</label>
        <input v-model='crypto_amount' type="decimal">
      </div>

      <div id="groupbox">
        <label for="cantidad">Ingrese cuanto desea vender:(ARS)</label>
        <input v-model='money' type="number">
      </div>

      <button id="btnVender" value="vender" @click="Venta">Vender</button>
...
export default {
  ...
  mounted(){      
    //const urlBitcoin = 'https://criptoya.com/api/argenbtc/btc/ars/0.5'
    //this.getApiCriptoYa(urlBitcoin);
    //this.getApiCriptoYa();
    console.log("User:" + localStorage.user_id);
  ...
  methods: {     
      Venta(){  
        ...
        if (localStorage.user_id == null){
          ...
          alert("Ha vendido " + this.crypto_amount + " " + this.crypto_code + " por " + this.money);
          this.postApi(saleData);    
      ...
      postApi(saleData){
        ...
        return apiClient.post('/transactions', saleData)
      },
